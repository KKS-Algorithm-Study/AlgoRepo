...
for _ in range(F):
    ...
    files[name] = {'permission':permission,'owner': owner,'group': owned_group}

Q = int(input())

def checkPermission(cur_set, up):
    if up == 1:
        cur_set.add('X')
    elif up == 2:
        cur_set.add('W')
...
for _ in range(Q):
    ...
    try:
        ...
        answer = 2
    s_permission = set()
    s_permission = checkPermission(s_permission, int(files[file_name]['permission'][answer]))
